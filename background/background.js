...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Request for batch size from content script
  if (message.action === 'getBatchSize') {
    chrome.storage.local.get(['batchSize'], (data) => {
      const batchSize = data.batchSize || 20; // Default to 20 if not set
      sendResponse({ batchSize });
    });
